...
| 모의 주문 | 부분 완료 | 논리 키 및 signalId 기반 요청 API, 승인/거절/BROKER_FAILED 결과, 동일 row 수동 재시도 API 존재. 자동 재시도와 signal_id 연결은 미구현 |
...
3. order_requests에 signal_id 연결 및 재시도 성공 시 신호 상태 동기화
4. RETRY_REQUESTED 장기 정체 복구 정책과 운영 조회 추가
