...
class LivePositionEntity {
    ...
    @Enumerated(EnumType.STRING) @Column(name = "trading_environment") KisEnvironment environment;
    ...
    void update(LivePosition v) { stockCode=v.stockCode(); environment=v.environment(); quantity=v.quantity(); averageBuyPrice=v.averageBuyPrice(); buyAmount=v.buyAmount(); buyCommission=v.buyCommission(); status=v.status(); openedAt=v.openedAt(); closedAt=v.closedAt(); }
    LivePosition toDomain() { return new LivePosition(id, stockCode, environment, quantity, averageBuyPrice, buyAmount, buyCommission, status, openedAt, closedAt); }
}
