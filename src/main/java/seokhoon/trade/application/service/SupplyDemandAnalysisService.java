...
public class SupplyDemandAnalysisService implements AnalyzeSupplyDemandUseCase {
    ...
    private static Map<LocalDate,Daily> aggregate(List<StockInvestorFlow> values){
        ...
        Map<LocalDate,Daily> result=new HashMap<>();for(var v:selected.values()){Daily d=result.computeIfAbsent(v.tradeDate(),x->new Daily());if(v.investorType()==InvestorType.FOREIGN)d.foreign=d.foreign.add(v.netBuyAmount());else if(v.investorType()==InvestorType.INDIVIDUAL)d.individual=d.individual.add(v.netBuyAmount());else if(INSTITUTIONS.contains(v.investorType()))d.institution=d.institution.add(v.netBuyAmount());}return result;}
    ...
    private static final class Daily {BigDecimal foreign=BigDecimal.ZERO,institution=BigDecimal.ZERO,individual=BigDecimal.ZERO;BigDecimal smart(){return foreign.add(institution);}}
}
