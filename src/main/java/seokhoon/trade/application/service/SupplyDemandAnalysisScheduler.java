...
public class SupplyDemandAnalysisScheduler {
    ...
    private final CorrelationIdProvider correlations; private final Clock clock;
    @org.springframework.beans.factory.annotation.Autowired
    public SupplyDemandAnalysisScheduler(AnalyzeSupplyDemandUseCase useCase,MarketCalendarPort calendar,
            SchedulerExecutionHistoryPort histories,OperationalMetricsPort metrics,CorrelationIdProvider correlations){this(useCase,calendar,histories,metrics,correlations,Clock.system(SEOUL));}
    SupplyDemandAnalysisScheduler(AnalyzeSupplyDemandUseCase useCase,MarketCalendarPort calendar,
            SchedulerExecutionHistoryPort histories,OperationalMetricsPort metrics,CorrelationIdProvider correlations,Clock clock){this.useCase=useCase;this.calendar=calendar;this.histories=histories;this.metrics=metrics;this.correlations=correlations;this.clock=clock;}
