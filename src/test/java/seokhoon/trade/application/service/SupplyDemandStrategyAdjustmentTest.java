...
class SupplyDemandStrategyAdjustmentTest {
    ...
    @Test void candidateAdjustmentHasNoProviderOrOrderDependency(){assertThat(Arrays.stream(SupplyDemandStrategyAdjustment.class.getDeclaredConstructors()).flatMap(value->Arrays.stream(value.getParameterTypes())).map(Class::getName)).noneMatch(value->value.contains("InvestorFlowProvider")||value.contains("Order")||value.contains("Broker"));}
